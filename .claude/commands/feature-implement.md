...
2. `ui-design-brief.md` を読み込み、デザイン参照方法を判定する:
...
implement.md に従い、以下が自動的に行われる:
- tasklist.md の未完了タスクを順に実装（各タスクでローカル検証）
- フェーズ完了時にエージェントをバッチ起動（code-reviewer + security-reviewer 等）
- 全タスク完了後に振り返りを記録

**注意**: フェーズ完了時のテスト・レビュー・振り返りは implement.md 内部で処理される。このコマンドでは重複して実行しない。

implement.md の全ステップ完了後、ステップ4に進む。
